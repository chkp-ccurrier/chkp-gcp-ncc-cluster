...
resource "google_compute_address" "c2_primary_clus_ext_ip_addr" {
  name = "s2-${var.primary_cluster_address_name}"
  region = var.region2
}

resource "google_compute_address" "c2_secondary_clus_ext_ip_addr" {
  name = "s2-${var.secondary_cluster_address_name}"
  region = var.region2
}
...
module "member_s2a" {
  ...
  prefix = var.prefix
  member_name = "s2-${var.prefix}-member-a"
  region = var.region2
  zone = var.zoneC
  ...
  cluster_network = google_compute_network.spoke_net_0.name
  cluster_network_subnetwork = google_compute_subnetwork.spoke_subnet_0.name
  cluster_ip = "${var.new_subnet_1_str[0]}${var.ha_a_ip}"
  mgmt_network = google_compute_network.new_net_2.name
  mgmt_network_subnetwork = google_compute_subnetwork.mgmt_subnet_2.name
  mgmt_ip = "${var.new_subnet_0_str[3]}${var.ha_a_ip}"
  num_internal_networks = var.num_internal_networks
  internal_network1_network = google_compute_network.spoke_net_1.name
  internal_network1_subnetwork = google_compute_subnetwork.spoke_subnet_1.name
  internal_network1_ip = "${var.new_subnet_1_str[2]}${var.ha_a_ip}"
  ...
  primary_cluster_address_name = google_compute_address.c2_primary_clus_ext_ip_addr.name
  secondary_cluster_address_name = google_compute_address.c2_secondary_clus_ext_ip_addr.name
  ...
  depends_on = [ google_compute_network.spoke_net_0,
                google_compute_subnetwork.new_subnet_2,
                ...
                google_compute_network.new_net_2,
                google_compute_subnetwork.mgmt_subnet_2,
                module.member_b
              ]
}
...
module "member_s2b" {
  ...
  prefix = var.prefix
  member_name = "s2-${var.prefix}-member-b"
  region = var.region2
  zone = var.zoneD
  machine_type = var.machine_type
  disk_size = var.bootDiskSizeGb
  ...
  cluster_network = google_compute_network.spoke_net_0.name
  cluster_network_subnetwork = google_compute_subnetwork.spoke_subnet_0.name
  cluster_ip = "${var.new_subnet_1_str[0]}${var.ha_b_ip}"
  mgmt_network = google_compute_network.new_net_2.name
  mgmt_network_subnetwork = google_compute_subnetwork.mgmt_subnet_2.name
  mgmt_ip = "${var.new_subnet_0_str[3]}${var.ha_b_ip}"
  num_internal_networks = var.num_internal_networks
  internal_network1_network = google_compute_network.spoke_net_1.name
  internal_network1_subnetwork = google_compute_subnetwork.spoke_subnet_1.name
  internal_network1_ip = "${var.new_subnet_1_str[2]}${var.ha_b_ip}"
/*  internal_network2_network = var.internal_network2_network
  internal_network2_subnetwork = var.internal_network2_subnetwork
  internal_network3_network = var.internal_network3_network
  internal_network3_subnetwork = var.internal_network3_subnetwork
