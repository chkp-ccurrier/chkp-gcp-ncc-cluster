locals {
  ...
  regex_admin_SSH_key = regex(local.regex_valid_admin_SSH_key, var.ssh_key) == var.ssh_key ? 0 : "Please enter a valid SSH public key or leave empty"
  ...
  admin_SSH_key_condition = var.ssh_key != "" ? true : false
}